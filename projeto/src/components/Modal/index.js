...
export const CancelattionModal = ({
  ...
  const CancelarConsulta = async (id) => {
    await api
      .put(`/Consultas/Status?idConsulta=${id}&status=Cancelada`)
      .then(() => {
        ListarConsultas();
      ...
      <PatientModal>
        <ModalContent>
          ...
          <ButtonModal onPress={() => CancelarConsulta()}>
            <ButtonTitle onPress={() => CancelarConsulta()}>
              Confirmar
            </ButtonTitle>
          </ButtonModal>
...
export const ConsultaModalCard = ({
  consulta,
  visible,
  setShowModal = null,
  navigation,
  ...
  const HandlePress = () => {
    setShowModal(false);
    navigation.replace("LocalConsulta", {
  ...
  useEffect(() => {
    if (perfil === "Medico") {
      setIdadePaciente(
        moment
          .duration(moment().diff(moment(consulta.paciente.dataNascimento)))
          .asYears()
      );
    }
  }, []);
      ...
      <PatientModal>
        <ModalContent>
          ...
          <ButtonModal onPress={HandlePress}>
            <ButtonTitle onPress={HandlePress}>
